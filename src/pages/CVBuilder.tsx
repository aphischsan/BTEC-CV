...
import { Download, FileText, Layout, Send, Sparkles, Trash2 } from 'lucide-react';
...
const STORAGE_KEY = 'btec-cv-data';

function freshDefaultData(): CVData {
  return JSON.parse(JSON.stringify(DEFAULT_CV_DATA)) as CVData;
}
...
export default function CVBuilder() {
  ...
  const clearCurrentCV = () => {
    if (!window.confirm('Clear this CV from this device? This cannot be undone.')) {
      return;
    }

    localStorage.removeItem(STORAGE_KEY);
    setData(freshDefaultData());
    setActiveTab('edit');
      ...
      <header className="h-16 bg-white border-b border-slate-200 px-4 sm:px-6 flex items-center justify-between shadow-sm shrink-0 print:hidden z-10">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <span className="text-sm text-slate-500 hidden lg:block">
            {submitSuccess ? <span className="text-green-600 font-medium">✓ CV Submitted!</span> : 'Auto-saved on this device'}
          </span>
          <button
            onClick={clearCurrentCV}
            className="px-3 py-2 bg-white border border-slate-300 text-slate-600 rounded-md font-medium flex items-center gap-2 text-sm hover:bg-slate-50 transition-colors"
            title="Clear this CV from this device"
          >
            <Trash2 size={16} />
            <span className="hidden xl:inline">Clear this CV</span>
          </button>
          <button
